...
function linear(x, w) {
    return w.map((row) => row.reduce((acc, wi, i) => acc.add(wi.mul(x[i])), new Value(0)));
}
...
function gptForward(tokenId, posId, keys, values, sd, nLayer, nHead, headDim, nEmbd) {
    ...
    for (let li = 0; li < nLayer; li++) {
        ...
        for (let h = 0; h < nHead; h++) {
            ...
            const attnLogits = kH.map((kt) => qH
                .reduce((acc, qj, j) => acc.add(qj.mul(kt[j])), new Value(0))
                .mul(scale));
            const attnWeights = softmax(attnLogits);
            for (let j = 0; j < headDim; j++) {
                xAttn.push(attnWeights.reduce((acc, w, t) => acc.add(w.mul(vH[t][j])), new Value(0)));
            }
        }
...
async function main() {
    ...
    for (let step = 0; step < numSteps; step++) {
        ...
        for (let posId = 0; posId < n; posId++) {
            ...
            const logits = gptForward(tokenId, posId, keys, values, sd, nLayer, nHead, headDim, nEmbd);
        ...
        process.stdout.write(`\rstep ${(step + 1).toString().padStart(4)} / ${numSteps} | loss ${loss.data.toFixed(4)}`);
    }
    // Inference
