...
function gptForward(tokenId, posId, keys, values, sd, nLayer, nHead, headDim, nEmbd, viz = null) {
    ...
    for (let li = 0; li < nLayer; li++) {
        ...
        for (let h = 0; h < nHead; h++) {
            ...
            if (viz) {
                if (!viz.headAttention[li]) viz.headAttention[li] = [];
                viz.headAttention[li][h] = attnWeights.map((w) => w.data);
            }
        ...
        if (viz) viz.attn.push(meanAbs(x));
        ...
        if (viz) viz.mlp.push(meanAbs(x));
    }
    const logits = linear(x, sd.lm_head);
    if (viz) {
        viz.logits = meanAbs(logits);
    }
    return logits;
}
...
function toNetworkNodes(viz, outputValue = 0) {
    const avgAttn = viz && viz.attn && viz.attn.length > 0
        ? viz.attn.reduce((a, b) => a + b, 0) / viz.attn.length
        : 0;
    const avgMlp = viz && viz.mlp && viz.mlp.length > 0
        ? viz.mlp.reduce((a, b) => a + b, 0) / viz.mlp.length
        : 0;
    return [
        { id: 'input', label: 'Input', value: 1 },
        { id: 'embed', label: 'Embed', value: viz ? viz.embed : 0 },
        { id: 'norm', label: 'Norm', value: viz ? viz.normIn : 0 },
        { id: 'attn', label: 'Attention', value: avgAttn },
        { id: 'mlp', label: 'MLP', value: avgMlp },
        { id: 'logits', label: 'Logits', value: viz ? viz.logits : 0 },
        { id: 'output', label: 'Output', value: outputValue }
    ];
}

function topKFromProbs(probData, k = 10) {
    return probData
        .map((prob, idx) => ({
            tokenId: idx,
            token: idx === BOS ? '<BOS>' : allChars[idx],
            prob
        }))
...
function train(learningRate, numSteps, temperature) {
    ...
    for (let step = 0; step < numSteps; step++) {
        ...
        postMessage({
            ...
            total: numSteps,
            loss: finalLoss,
            nodes: toNetworkNodes(stepViz, finalLoss > 0 ? 1 / (1 + finalLoss) : 1)
        });
    }
...
function infer(temperature, count) {
    ...
    for (let sampleIdx = 0; sampleIdx < count; sampleIdx++) {
        ...
        for (let posId = 0; posId < blockSize; posId++) {
            ...
            const rawLogits = logits.map((l) => l.data);
            const probData = probs.map(p => p.data);
            tokenId = rng.choices(probData);
            const chosenProb = probData[tokenId] || 0;
            const topK = topKFromProbs(probData, 12);
            trace.push({
                tokenIndex: posId + 1,
                inputToken: inputTokenId === BOS ? '<BOS>' : allChars[inputTokenId],
                outputToken: tokenId === BOS ? '<BOS>' : allChars[tokenId],
                confidence: chosenProb,
                rawLogits,
                topK,
                nodes: toNetworkNodes(viz, chosenProb),
                stages: {
                    embed: viz.embed || 0,
                    normIn: viz.normIn || 0,
                    attn: viz.attn || [],
